...
==============================================================
  ...
  Full Integration of All Projects in One System
==============================================================

This is the main entry point that runs everything in the correct order:

  ✅ Project 5 → Real-world data analysis
  ✅ Project 1 → Full pipeline from data to ML model
  ✅ Project 2 → Regression: predict numeric grade
  ✅ Project 3 → Classification: classify student level
  ✅ Project 4 → LLM Agent: AI-powered study assistant
  ✅ Project 6 → Visualization: professional charts & dashboards

HOW TO RUN:
  python run_all.py

OUTPUT:
  All output files will be saved in the outputs/ folder
==============================================================
"""
...
# ══════════════════════════════════════════════════════════════
section("STEP 0 — Generate Student Data")

data_path = os.path.join(BASE_DIR, "data", "student_performance.csv")

if os.path.exists(data_path):
    ok(f"Data already exists → {data_path}")
else:
    step("Generating student data ...")
    import subprocess
...
# ══════════════════════════════════════════════════════════════
# Helper function to run any project
# ══════════════════════════════════════════════════════════════
def run_project(script_path, working_dir, project_name):
    """Run a specific project script and display the result."""
    step(f"Running {project_name} ...")
    t0 = time.time()
    result = subprocess.run(
    ...
    if result.returncode == 0:
        ok(f"{project_name} completed in {elapsed:.1f} seconds")
        lines = result.stdout.strip().split("\n")
        for line in lines[-8:]:
            print(f"     {line}")
    else:
        warn(f"{project_name} failed!")
...
def load_and_run_p5():
    """Load and run project5_analysis.py with adjusted paths."""
    import pandas as pd
    import numpy as np
    import matplotlib
    matplotlib.use("Agg")
    ...
    step(f"Project 5 data: {df.shape[0]} students, {df.shape[1]} columns")
...
def run_p1(df_clean):
    ...
    ok(f"Best model: {best_name}  (R²={best['R2']})")
...
# ══════════════════════════════════════════════════════════════
# PROJECT 2 — Regression (Predict Final Score)
# ══════════════════════════════════════════════════════════════
...
def run_p2(df_clean):
    ...
    best = results[best_name]
    ok(f"Best model: {best_name}  (R²={best['r2']:.3f})")

    joblib.dump(best["pipe"], os.path.join(MODELS_DIR, "p2_regression.joblib"))
    ok(f"Model saved → models/p2_regression.joblib")
...
def run_p3(df_clean):
    ...
    best = results[best_name]
    ok(f"Best model: {best_name}  (F1={best['f1']:.3f})")

    joblib.dump(best["pipe"], os.path.join(MODELS_DIR, "p3_classifier.joblib"))
    ok(f"Model saved → models/p3_classifier.joblib")
    return best["pipe"], NUM_FEATS
...
# ══════════════════════════════════════════════════════════════
# PROJECT 4 — LLM Agent (Smart Study Assistant)
# ══════════════════════════════════════════════════════════════
section("PROJECT 4 — LLM Study Agent")

def run_p4_demo():
    """Run a demo of the Agent without an API key (mock mode)."""
    import json

    MOCK_RESPONSE = {
        "summary": "The student spent 2 hours learning Python and understood variables and functions well, but struggled with loops.",
        "strengths": ["Completed variables and functions chapters", "Studied consistently for 2 full hours"],
        "weaknesses": ["Difficulty with for loops and range()", "Did not complete all exercises"],
        "next_session_plan": {
            "focus_topic": "For loops and iteration in Python",
            "recommended_activities": [
                "Watch a dedicated video on Python loops on YouTube",
                "Solve 5 HackerRank Easy exercises",
                "Rewrite previous exercises using loops"
            ],
            "estimated_time_minutes": 60
        },
        "motivation_message": "Struggling with loops is totally normal — one focused session is enough to break through this barrier!"
    }

    api_key = os.getenv("ANTHROPIC_API_KEY", "")
    if api_key:
        step("ANTHROPIC_API_KEY found — will use the real API")
    else:
        step("No API Key found — running in Mock Demo mode")

    step("Sample Agent response:")
    print()
    print(f"     📝 Summary: {MOCK_RESPONSE['summary']}")
    print(f"\n     ✅ Strengths:")
    ...
    print(f"\n     📅 Next Session ({plan['estimated_time_minutes']} minutes):")
    print(f"        Topic: {plan['focus_topic']}")
    for act in plan["recommended_activities"]:
        print(f"        → {act}")
    print(f"\n     💬 {MOCK_RESPONSE['motivation_message']}")
    ok("Project 4 ready — to enable real AI: export ANTHROPIC_API_KEY=your_key")
...
def run_p6(df_clean, p2_model, p3_model, feat_cols):
    ...
    if len(at_risk) > 0:
        ...
        ax.hist(current, bins=15, alpha=0.6, color=PALETTE["at_risk"],
                label=f"Current (mean={current.mean():.1f})")
        ax.hist(improved_preds, bins=15, alpha=0.6, color=PALETTE["strong"],
                label=f"After Improvement (mean={improved_preds.mean():.1f})")
...
# ══════════════════════════════════════════════════════════════
# PROJECT 7 — Integrated System: Analyze a New Student
# ══════════════════════════════════════════════════════════════
...
def run_integrated_demo(p2_model, p3_model, feat_cols, df_all):
    ...
    print(f"  │  Attendance Rate   : {student['attendance_pct']}%                        │")
    print(f"  │  Assignments Done  : {student['assignments_done']} / 20                       │")
    print(f"  │  Missed Deadlines  : {student['missed_deadlines']}                           │")
    print("  ├─────────────────────────────────────────────────────┤")
    print(f"  │  🎯 Predicted Score : {pred_score} / 100                    │")
    print(f"  │  📊 Classification  : {pred_label.upper():<20}           │")
    print("  ├─────────────────────────────────────────────────────┤")
    ...
    advice = {
        "at_risk": [
            "⚠️  You are in the danger zone — but you can improve quickly",
            "   → Increase daily study hours to at least 5 hours",
            "   → Attending classes is mandatory — attendance is one of the strongest predictors",
            "   → Complete all overdue assignments before starting new material",
        ],
        "average": [
            "📈 Your performance is average — you are close to a good level",
            "   → Focus on pushing assignment completion above 80%",
            "   → Revision sessions significantly improve your grade",
            "   → Maintain consistent attendance",
        ],
        "strong": [
            "🏆 Excellent performance! You are among the top students",
            "   → Keep the same study routine",
            "   → Consider helping your peers with understanding",
            "   → Start final revision early to maintain your level",
        ],
    }
    print()
    ...
    plt.close()
    ok("p7_student_dashboard.png saved")
...
# FINAL SUMMARY
# ══════════════════════════════════════════════════════════════
section("✅ All Projects Completed Successfully!")
...
print(f"\n  📁 Saved ML models in models/ folder:")
models_dir = os.path.join(BASE_DIR, "models")
...
print("""
  ══════════════════════════════════════════════════════
  Summary:
    Project 5 → Analyzed 500 real student records
    Project 1 → Built a full end-to-end ML pipeline
    Project 2 → Regression model predicts numeric grade
    Project 3 → Classification model predicts student level
    Project 4 → AI Agent providing personalized study advice
    Project 6 → 4 professional visualization dashboards
    Project 7 → Integrated system to analyze any new student
  ══════════════════════════════════════════════════════
""")
